...
const PetDietSection = ({ pet }) => {
  const recommended =
    Array.isArray(pet?.recommendedDiet) && pet.recommendedDiet.length > 0
      ? pet.recommendedDiet
      : RECOMMENDED_DIET;
  const avoid =
    Array.isArray(pet?.foodsToAvoid) && pet.foodsToAvoid.length > 0 ? pet.foodsToAvoid : FOODS_TO_AVOID;
